...
> 编写 worker 的完整机制与编写指南(三档基类、声明式监听、`to_round` 例子、测试)见 **[docs/v1/worker.md](docs/v1/worker.md)**。本节只给底层裸端口契约。

**worker = adapter**:实现「怎么发现有新数据」「怎么增量读」「怎么归一」,engine 负责把它们串成同步循环。契约原样保留自 `adapters/base.py`(剥离时几乎逐字带走)。**多数作者不直接写这四个方法**,而是继承 `JsonlWorker` / `FileWorker` 只写 `to_round`(docs/v1/worker.md §1);裸端口留给 HTTP / 异形来源:
...
> 字段全表、构造器(`Round` / `Text` / `ToolUse` …)、幂等规则、role/speaker 分类经验见 **[docs/v1/session-format.md](docs/v1/session-format.md)**。
