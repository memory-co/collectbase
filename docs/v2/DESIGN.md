...
- **层分支只放这一层的内容** ⇒ 它的树 ≠ stack 的树
- **SHA 不变** ⇒ 那个提交对象必须原封不动地成为权威提交

在 stack 上提交的话,那个对象的树里带着所有层的内容,搬到层分支就必然产生**新对象**——那是 cherry-pick,SHA 就变了。所以 `stack` 只接收 merge,`commit-msg` 直接拒绝在它上面提交,并告诉你切到哪。

代价:`layer/beliefs` 的工作区是**始祖 + beliefs 自己的文件**,看不到 init 之后新增的事实。要读着事实写结论,得另想办法(多开一个 stack 的 worktree 只读、或者用 `cb serve` 的文件管理器)。这条记在 §16。
