...
```
                 ┌── layer/facts     ●──●──●     权威。线性,必须 fast-forward
始祖 ●───────────┼── layer/notes     ●──●
(写 layers 的     └── layer/beliefs   ●──●──●
 那个提交)
                    stack           ●──●──●──●  合并视图。全是 merge
                                                 树 = 各层的合并,由 git 自己算
                                                 每个 merge 的信息 = 那次权威提交的信息
```

**所有层分支都从始祖出发。**共同祖先在那儿,merge 的基就有了:各层此后各加各的文件,是相对这个基的不相交改动,git 算并集时那份共同的基是**基**而不是重复,不会撞。所以 `cb init` 只造一个提交,其余全是 ref——没有孤儿分支,也不需要我们拼并集树。

于是"这一层的东西"= **相对始祖的增量**(`git diff --diff-filter=A <始祖> layer/notes`),归属判定也按这个来。

- **`layer/<name>` 是唯一的权威。** 每条只放自己这一层的文件,历史线性、无 merge、必须 FF。
- **`stack` 是构建产物。** 一串 merge 节点。**并集由 git 的 merge 自己算**——两侧改动不相交,天然无冲突;冲突就意味着 I1 被破坏了,那是损坏信号不是日常。
- **`stack` 名字固定。** 加一层不会让它改名,所以没人需要为此切分支。

### 只有一个入口

**提交打在 `layer/<L>` 上。**`post-commit` 把它 merge 进 `stack` —— 原样,同一个 SHA(§6)。`stack` 只接收 merge,在它上面提交会被拒绝。

`git log --first-parent stack` 读起来就是那条时间线本身,每一行都是某次权威提交的信息。
...
| **`stack`**(构建产物) | 新增的每个提交都必须是 **merge 节点**且带合法 `[层名]`;内容不重查(已在权威分支那侧验过);不要求 FF(`cb rebuild` 会整条重造) |
| 其他分支 | 完全放行 |

`stack` 上"必须是 merge 节点"这一条就把 `--no-verify` 堵死了:绕过 `commit-msg` 在 stack 上提交出来的是普通提交,ref 守卫直接拒。
...
```
1. 在 stack 上提交                       → 拒绝,告诉你切到哪条权威分支
2. [层名] 与所在分支不符                  → 拒绝
3. 无 [层名] / 层名不在 layers 里          → 拒绝(层名按更新前的 layers 判定)
4. 改动的路径已归属于别的层                → 拒绝,并指出归谁
```
...
## 6. merge:stack 只接收 merge

`post-commit` 只干一件事:

```
stack = commit-tree <合并树> -p <stack> -p <你刚才那个提交>     信息逐字复制
```

**你那个提交原封不动地进了 stack**,同一个 SHA。`git merge-base --is-ancestor <你的提交> stack` 恒为真。合并树由 `git merge-tree` 算——各层从始祖出发,两侧改动不相交,永不冲突。

### 站在哪条分支上,结果都一样

`git add` + `git commit` 之后一定是:**改动落在 `layer/<声明的层>` 上,然后 merge 进 `stack`**。信息里的 `[层名]` 说了算,你站在哪无所谓。

- **站在 `layer/<L>` 上** —— 你那个提交**就是**权威对象,SHA 原样带进 stack。
- **站在 `stack` 上** —— 你那个对象带着所有层的内容,不可能同时是这一层的权威提交;在 `layer/<L>` 上重造一个,你那个草稿随后被退回。它从来不是权威的。

无论哪种,`git merge-base --is-ancestor <权威提交> stack` 恒为真,而且工作区在这一切之后仍然是干净的(merge 的树 = 你提交的树)。

**唯一的例外**:站在 `layer/notes` 上却声明 `[beliefs]`。把它挪走要非 FF 地退回 `layer/notes`,而权威分支必须线性只进不退——所以这一种直接拒绝,并告诉你切到 `layer/beliefs` 或去 `stack` 上提交。

> 于是 `stack` 是"什么都能写"的那个工作区:它看得见所有层,声明哪层都行。智能体读着事实写结论,就待在这里。
...
cb serve                    可选:路径 CRUD + 分层文件管理器(见 works/server.md)
```
