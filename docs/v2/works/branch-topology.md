...
但权威**不等于**投影器无条件服从。`reference-transaction` 挡住了经由 git 命令的一切越界(见 [`cli.md`](cli.md) §4),但直接手写 `.git/refs/…` 仍能把一个越界提交塞上写入面。这时:
...
> 这是本设计里**唯一**一处需要人介入的状态。经由 git 命令走不到这里,只有绕开 ref 事务直接写 `.git/` 才可能产生。
...
**② 地板不是结构性的,但也不再是纸做的。** 写入面的树里就有事实层的文件,智能体够得着。防线是:`chmod a-w`(挡普通写)→ `commit-msg`(快速反馈,可被 `--no-verify` 跳过)→ **`reference-transaction`(检查落进 ref 的每个提交,`--no-verify` 跳不过)**。第三道是真正的闸,细节与实测见 [`cli.md`](cli.md) §4。

剩下的绕法只有"直接手写 `.git/refs/…`"和"拆掉 `core.hooksPath`",那是拆机制不是绕机制。
