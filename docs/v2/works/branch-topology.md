...
> 投影器**停下来报错**,把写入面标记为 dirty,既不静默照做(那会把改动写进 `layer/facts`,智能体就真的改到了事实层),也不静默回滚(那会丢工作)。`cb check` 报 divergence,`cb sync` 拒绝继续,等人处理。
...
- **§7 配置** — `collectbase.toml` 的层名现在同时是提交信息里的标签,需要约束字符集(`[a-z][a-z0-9_-]*`)。
- **§9 CLI** — 去掉 `cb only`(`git checkout layer/x` 就是);`cb sync` 语义变成"投影 + 重算";新增 `cb check` 的 I1–I4。
