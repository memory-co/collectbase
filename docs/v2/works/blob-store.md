...
- **软链必须是相对路径且不逃逸出 `blob/`**。实测上层可以提交 `project/leak.txt -> /etc/hostname` 这样的软链,git 照收不误;而本机制假定"所有 `120000` 条目都是 blob 指针",所以这条要由 `reference-transaction` 守卫在入口拒掉,`cb check` 的 I5 复查。
